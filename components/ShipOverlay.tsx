...
void main() {
    ...
    if(t < MAX_DIST) {
        ...
        // Base Material
        vec3 albedo = vec3(0.12, 0.15, 0.2); // Darker, metallic blue-grey
        float shininess = 48.0;
        float specIntensity = 2.0;
        ...
        vec3 rimColor = vec3(0.4, 0.8, 1.0); // Cyan rim

        // Environment / Sky Reflection (Fake)
        // Reflect view direction around normal
        vec3 refDir = reflect(-viewDir, n);
        // Simple gradient for sky/ground reflection
        float skyRef = smoothstep(-0.2, 0.5, refDir.y); 
        vec3 envColor = mix(vec3(0.05, 0.05, 0.08), vec3(0.1, 0.2, 0.4), skyRef);

        // --- Composition ---
        
        // Ambient + Diffuse
        vec3 col = albedo * (vec3(0.05) + vec3(1.0)*diff) * ao;
        
        // Add Environment Reflection
        col += envColor * 0.3 * ao * (1.0 - diff); // More reflection in shadows
        
        // Add Specular
        col += vec3(1.0, 0.95, 0.9) * spec * specIntensity * ao;
        
        // Add Rim
        col += rimColor * fresnel * 1.5 * ao;
